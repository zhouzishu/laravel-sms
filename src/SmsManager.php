...
class SmsManager
{
     ...
     * @param array       $input
     */
    public function __construct(EasySMS $easySms, $token = null, array $input = [])
    {
     ...
     * @return array
     */
    public function requestVerifySms($data = [])
    {
    ...
    protected static function getStorageClassName()
    {
        ...
        if ($middleware === 'web' || (is_array($middleware) && in_array('web', $middleware))) {
            return 'Zhouzishu\LaravelSms\SessionStorage';
        }
        if ($middleware === 'api' || (is_array($middleware) && in_array('api', $middleware))) {
            return 'Zhouzishu\LaravelSms\CacheStorage';
        }
